...
class MateriController extends BaseController
{
    public function index()
    {
        ...
        $data['activePage'] = 'materi';
        return view('admin/data_materi/index', $data);
    }
    ...
    public function create()
    {
        ...
        $data['activePage'] = 'tambah_materi';
        return view('admin/data_materi/tambah', $data);
    }
    ...
    // Pindahkan file ke direktori yang diinginkan jika ada
        ...
        if ($fileMateri && $fileMateri->isValid() && !in_array($fileMateri->getClientMimeType(), $allowedFileTypes)) {
            return redirect()->to('admin/materi')->withInput()->with('gagal', 'File Materi harus berupa PDF, JPG, atau PNG');
        }
    ...
    // Validasi minimal salah satu file diisi
        if (!$fileMateri && (!$videoMateri || !$videoMateri->isValid()) && (!$audioMateri || !$audioMateri->isValid())) {
            return redirect()->to('admin/materi')->withInput()->with('gagal', 'Minimal salah satu dari file materi, video, atau audio harus diisi.');
        }
    ...
    // Pindahkan file video ke direktori yang diinginkan jika ada
        ...
        // Validasi tipe file video_materi
            ...
            if (!in_array($videoMateri->getClientMimeType(), $allowedVideoTypes)) {
                return redirect()->to('admin/materi')->withInput()->with('gagal', 'File Video harus berupa MP4 atau MOV.');
            }
    ...
    public function edit($id)
    {
        ...
        $data['activePage'] = 'edit_materi';
        return view('admin/data_materi/edit', $data);
    }
    ...
    public function update($id)
    {
        ...
        if ($fileMateri && $fileMateri->isValid()) {
            ...
            if (!in_array($fileMateri->getClientMimeType(), $allowedFileTypes)) {
                return redirect()->to('admin/materi')->withInput()->with('gagal', 'File Materi harus berupa PDF, JPG dan PNG');
            }
    ...
    public function delete($id)
    {
        ...
        if (!$materi) {
            return redirect()->to('admin/materi')->with('gagal', 'Data Materi tidak ditemukan.');
        }
    ...
    // Cek tipe konten untuk menentukan apakah itu video, audio, atau file lainnya
        ...
        // Jika itu video, tampilkan preview menggunakan tag <video>
            return view('admin/data_materi/video_view', ['videoPath' => $path]);
        } elseif (strpos($contentType, 'audio') === 0) {
        // Jika itu audio, tampilkan preview menggunakan tag <audio>
            return view('admin/data_materi/audio_view', ['audioPath' => $path]);
        } else {
        // Jika itu bukan video atau audio, tampilkan view dengan data file
            return view('admin/data_materi/materi_view', ['fileContent' => $fileContent, 'filename' => $filename]);
        }
    }
    ...
    public function viewVideo($filename)
    {
        ...
        $data['activePage'] = 'materi';
        return view('admin/data_materi/video_view', $data);
    }
    ...
    public function viewAudio($filename)
    {
        ...
        $data['activePage'] = 'materi';
        return view('admin/data_materi/audio_view', $data);
    }
}
